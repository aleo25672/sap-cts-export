...
* Radios / checkboxes have no DDIC field labels — use COMMENT + INITIALIZATION
* so texts show even when only the .abap source is pasted (no text pool).
SELECTION-SCREEN BEGIN OF BLOCK b2 WITH FRAME TITLE TEXT-002.
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS p_gui RADIOBUTTON GROUP out DEFAULT 'X' USER-COMMAND out.
SELECTION-SCREEN COMMENT 3(50) c_gui FOR FIELD p_gui.
...
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS p_hdr AS CHECKBOX DEFAULT 'X'.
SELECTION-SCREEN COMMENT 3(50) c_hdr FOR FIELD p_hdr.
SELECTION-SCREEN END OF LINE.
SELECTION-SCREEN BEGIN OF LINE.
PARAMETERS p_obj AS CHECKBOX DEFAULT 'X'.
