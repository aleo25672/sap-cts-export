# ABAP ICF handler — `ZCL_CTS_EXTRACT_ICF`

Exposes `CTS_API_READ_CHANGE_REQUEST` over **HTTP** so clients (including the Node `http` adapter) do not need RFC.

## Install

1. Create class **`ZCL_CTS_EXTRACT_ICF`** (public, final) in SE24 / ADT and paste [`zcl_cts_extract_icf.clas.abap`](./zcl_cts_extract_icf.clas.abap).
2. Activate (requires **`/UI2/CL_JSON`**).
3. **SICF** → `default_host` → `sap` → `bc` → create service **`zcts_extract`**.
4. Handler List → `ZCL_CTS_EXTRACT_ICF`.
...
```text
https://<host>:<icm-port>/sap/bc/zcts_extract?sap-client=100
```
...
```http
GET /sap/bc/zcts_extract?request=S4HK900123&format=json
GET /sap/bc/zcts_extract?dateFrom=20260301&dateTo=20260430&owner=DEVELOPER1&format=csv
```
